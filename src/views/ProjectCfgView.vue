...
onMounted(async () => {
    let projectId = currentRoute.params.id as string;
    let projectRuntimeDetailFromBackend= await invoke<ProjectRuntimeDetail>(
        GET_PROJECT_RUNTIME_DETAIL_CMD,
        { projectId: projectId }
    );
    projectRuntimeDetail.value = projectRuntimeDetailFromBackend;
});

watch(
    () => currentRoute.params.id as string,
    async (newProjectId, _) => {
        let projectRuntimeDetailFromBackend= await invoke<ProjectRuntimeDetail>(
            GET_PROJECT_RUNTIME_DETAIL_CMD,

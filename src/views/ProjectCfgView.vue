...
let currentProject = ref<ProjectState | undefined>(applicationState?.projects.get(currentRoute.params.id as string));

watch(() => currentRoute.params.id as string, (newProjectId, _) => {
    currentProject.value = applicationState?.projects.get(newProjectId)
});
